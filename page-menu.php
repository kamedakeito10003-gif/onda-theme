...
※**注意：** 上記コード内の `iframe src="..."` の部分は、現在 `index.php` で設定している**正しいGoogleマップのURL**に書き換えてから使ってください。

---

### 2. ファイルごとの修正手順

それぞれのファイルを開き、適切な位置にコードを貼り付けます。
基本的には、**ファイルの最後の方にある `</div>` の直前** に貼り付けます。

#### ① お知らせ一覧 (`archive.php`) と お知らせ詳細 (`single.php`)
この2つのファイルには、まだ何もありません。
ファイルの末尾にある `</div>` と `</main>` の間あたりを探し、上記のコードを丸ごと貼り付けます。
